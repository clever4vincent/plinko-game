...
<div class="relative">
  <div class="mx-auto flex h-full flex-col pb-4" style:max-width={`${WIDTH}px`}>
    <div class="relative w-full" style:aspect-ratio={`${WIDTH} / ${HEIGHT}`}>
      {#if $plinkoEngine === null}
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <CircleNotch class="size-20 animate-spin text-[#f9d846]" weight="bold" />
        </div>
      {/if}
  ...
  <div class="absolute top-1/5 right-[5%] -translate-y-1/2">
    <LastWins />
  </div>
</div>
